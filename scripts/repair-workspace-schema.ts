...
try {
  ...
  await client.query(`
    ...
    DO $$ BEGIN IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname='DocumentVerificationStatus') THEN CREATE TYPE "DocumentVerificationStatus" AS ENUM ('PENDING','VERIFIED','REJECTED','EXPIRED'); END IF; END $$;
    CREATE TABLE IF NOT EXISTS "EmployeeDocument" ("id" TEXT NOT NULL,"companyId" TEXT NOT NULL,"employeeId" TEXT NOT NULL,"documentType" TEXT NOT NULL,"title" TEXT NOT NULL,"objectKey" TEXT NOT NULL,"fileName" TEXT NOT NULL,"mimeType" TEXT NOT NULL,"sizeBytes" INTEGER NOT NULL,"issueDate" DATE,"expiryDate" DATE,"verificationStatus" "DocumentVerificationStatus" NOT NULL DEFAULT 'PENDING',"verifiedById" TEXT,"verifiedAt" TIMESTAMP(3),"rejectionReason" TEXT,"retentionUntil" DATE,"createdAt" TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP,"updatedAt" TIMESTAMP(3) NOT NULL,CONSTRAINT "EmployeeDocument_pkey" PRIMARY KEY("id"));
    CREATE INDEX IF NOT EXISTS "EmployeeDocument_companyId_expiryDate_idx" ON "EmployeeDocument"("companyId","expiryDate");
    CREATE INDEX IF NOT EXISTS "EmployeeDocument_employeeId_documentType_idx" ON "EmployeeDocument"("employeeId","documentType");
  ...
  await constraint('EmployeeDocument_companyId_fkey','ALTER TABLE "EmployeeDocument" ADD CONSTRAINT "EmployeeDocument_companyId_fkey" FOREIGN KEY("companyId") REFERENCES "Company"("id") ON DELETE CASCADE ON UPDATE CASCADE');
  await constraint('EmployeeDocument_employeeId_fkey','ALTER TABLE "EmployeeDocument" ADD CONSTRAINT "EmployeeDocument_employeeId_fkey" FOREIGN KEY("employeeId") REFERENCES "Employee"("id") ON DELETE CASCADE ON UPDATE CASCADE');
