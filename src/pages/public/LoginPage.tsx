...
import { 
  ...
  AlertCircle, CheckCircle2, User, Sparkles
} from 'lucide-react';
...
export const LoginPage: React.FC<LoginPageProps> = ({
  ...
  const { login, loginAsDemoUser, companies } = useAuth();
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-8 relative font-sans selection:bg-brand-500 selection:text-white">
      {/* Background glowing gradients */}
      <div className="absolute top-1/3 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[400px] bg-gradient-to-tr from-brand-600/15 via-purple-600/15 to-transparent blur-[120px] pointer-events-none rounded-full" />
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-slate-900/90 backdrop-blur-xl py-8 px-6 sm:px-8 border border-slate-800 rounded-3xl shadow-2xl space-y-6">
          ...
          <form onSubmit={handleSubmit} className="space-y-4 text-xs">
            <div>
              <label className="block font-medium text-slate-300 mb-1">Work / Admin Email</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl pl-9 pr-3.5 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:border-brand-500 text-xs"
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="••••••••••••"
                  className="w-full bg-slate-800 border border-slate-700 rounded-xl pl-9 pr-10 py-2.5 text-white placeholder-slate-500 focus:outline-none focus:border-brand-500 text-xs font-mono"
                />
                <button
                  type="button"
            ...
            <button
              ...
              className="w-full py-3 rounded-xl text-xs font-bold text-white bg-gradient-to-r from-brand-500 to-indigo-600 hover:from-brand-600 hover:to-indigo-700 shadow-md shadow-brand-500/25 transition-all flex items-center justify-center space-x-1.5 disabled:opacity-50"
            >
              {isSubmitting ? (
          ...
          {/* If there are registered users in database, show quick select */}
          {registeredUsers.length > 0 && (
            <div className="pt-4 border-t border-slate-800 space-y-2.5">
              <div className="text-[11px] font-bold text-slate-400 uppercase tracking-wider flex items-center space-x-1.5">
                ...
                <span>Registered Workspace Accounts ({registeredUsers.length})</span>
              </div>
              ...
              <div className="space-y-1.5 max-h-40 overflow-y-auto">
                {registeredUsers.map((u) => (
                  <button
                    ...
                    className="w-full p-2 rounded-xl bg-slate-800/60 hover:bg-slate-800 border border-slate-700/60 flex items-center justify-between text-left transition-all"
                  >
                    <div>
                      <div className="text-xs font-bold text-white truncate">{u.fullName}</div>
                      <div className="text-[10px] text-slate-400 font-mono">{u.email}</div>
                    </div>
                    <span className="text-[9px] px-1.5 py-0.5 rounded bg-brand-500/20 text-brand-300 font-semibold">
                      {u.role}
                    </span>
                  </button>
                ))}
      ...
      {showForgotModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-md bg-slate-900 border border-slate-700 rounded-3xl p-6 shadow-2xl z-10 animate-slide-up text-xs">
            ...
            {forgotSent ? (
              <div className="mt-4 p-4 rounded-xl bg-emerald-500/10 border border-emerald-500/30 text-emerald-300">
                ...
                <button
                  ...
                  className="mt-4 w-full py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-xl font-semibold"
                >
                  Return to Login
                </button>
              </div>
              ...
              >
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <button
                    type="submit"
                    className="flex-1 py-2 bg-brand-500 hover:bg-brand-600 text-white rounded-xl font-semibold shadow"
                  >
                    Send Reset Link
                  </button>
                </div>
