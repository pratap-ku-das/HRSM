...
export const ResetPasswordPage: React.FC<{ onNavigateToLogin: () => void }> = ({ onNavigateToLogin }) => {
  ...
  const submit = async (event: React.FormEvent) => {
    ...
    if (!token) return setError('This reset link is missing its token. Please use the complete link from your email.');
  ...
  return <main className="min-h-screen bg-slate-950 text-white flex items-center justify-center px-4">
    <section className="w-full max-w-md rounded-3xl border border-white/10 bg-slate-900 p-8 shadow-2xl">
      ...
      {complete ? <div className="mt-8 text-center">
        <CheckCircle2 className="mx-auto h-14 w-14 text-emerald-400" />
        <h1 className="mt-4 text-2xl font-bold">Password updated</h1>
        <p className="mt-2 text-sm text-slate-400">Your new password is ready. Sign in again on the web or Android app.</p>
        ...
        <div className="mt-6 text-center"><ShieldCheck className="mx-auto h-10 w-10 text-brand-400" /><h1 className="mt-3 text-2xl font-bold">Choose a new password</h1><p className="mt-2 text-sm text-slate-400">Use at least 10 characters.</p></div>
        ...
        <form onSubmit={submit} className="mt-6 space-y-4">
          ...
          <button disabled={submitting || !token} className="w-full rounded-xl bg-brand-600 px-4 py-3 font-semibold hover:bg-brand-500 disabled:opacity-50">{submitting ? 'Updating…' : 'Update password'}</button>
        </form>
      </>}
    </section>
  </main>;
