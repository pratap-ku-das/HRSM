...
export const DashboardOverview: React.FC<{ setActiveView: (view: string) => void }> = ({ setActiveView }) => {
  ...
  const load = useCallback(async () => {
    setBusy(true);
    const results = await Promise.allSettled([api.getCommandCenter(), api.getApprovalInbox(), api.getNotifications(true), api.getOperationsWorkspace()]);
