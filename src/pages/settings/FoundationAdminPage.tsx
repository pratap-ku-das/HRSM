...
type OrgKind = 'branches' | 'locations' | 'departments' | 'designations' | 'teams' | 'cost-centers' | 'grades';
type DisplayItem = { id: string; name?: string; code?: string; title?: string; gradeLevel?: string };
...
export const FoundationAdminPage: React.FC = () => {
  ...
  const collections = useMemo(() => organization ? {
    branches: organization.branches, locations: organization.locations, departments: organization.departments,
    designations: organization.designations, teams: organization.teams,
  ...
  const createOrg = async (event: React.FormEvent) => {
    ...
    try {
      const body: Record<string, unknown> = kind === 'designations'
        ? { departmentId: parentId, title: name, gradeLevel: code, minSalary: 0, maxSalary: 0 }
        : { name, code };
      if (kind === 'designations' && !parentId) throw new Error('Choose the designation department.');
  ...
  return <div className="neo-page neo-foundation space-y-5">
    ...
    {tab === 'organization' && <div className="grid xl:grid-cols-[340px_1fr] gap-5">
      <form onSubmit={createOrg} className="bg-slate-900 border border-slate-800 rounded-2xl p-5 space-y-3"><h2 className="font-bold flex items-center gap-2"><Plus className="w-4 h-4"/>Add structure item</h2><select value={kind} onChange={e => { setKind(e.target.value as OrgKind); setParentId(''); }} className="w-full bg-slate-950 border border-slate-700 rounded-xl p-2.5">{(['branches','locations','departments','designations','teams','cost-centers','grades'] as OrgKind[]).map(x => <option key={x} value={x}>{x}</option>)}</select><input required value={name} onChange={e => setName(e.target.value)} placeholder={kind === 'designations' ? 'Designation title' : 'Name'} className="w-full bg-slate-950 border border-slate-700 rounded-xl p-2.5"/><input required value={code} onChange={e => setCode(e.target.value)} placeholder={kind === 'designations' ? 'Grade level, e.g. L3' : 'Code'} className="w-full bg-slate-950 border border-slate-700 rounded-xl p-2.5"/>{kind === 'locations' && <select value={parentId} onChange={e => setParentId(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-xl p-2.5"><option value="">No branch</option>{organization?.branches.map(x => <option key={x.id} value={x.id}>{x.name}</option>)}</select>}{kind === 'designations' && <select required value={parentId} onChange={e => setParentId(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-xl p-2.5"><option value="">Choose department</option>{organization?.departments.map(x => <option key={x.id} value={x.id}>{x.name}</option>)}</select>}{kind === 'teams' && <select value={parentId} onChange={e => setParentId(e.target.value)} className="w-full bg-slate-950 border border-slate-700 rounded-xl p-2.5"><option value="">No department</option>{organization?.departments.map(x => <option key={x.id} value={x.id}>{x.name}</option>)}</select>}<button disabled={busy} className="w-full bg-brand-500 rounded-xl p-2.5 font-bold">Create</button></form>
      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-3">{collections && (Object.entries(collections) as [string, DisplayItem[]][]).map(([label, items]) => <section key={label} className="bg-slate-900 border border-slate-800 rounded-2xl p-4"><h3 className="font-bold capitalize flex items-center gap-2"><Building2 className="w-4 h-4 text-brand-400"/>{label} <span className="text-slate-500">({items.length})</span></h3><div className="mt-3 space-y-2">{items.map(item => <div key={item.id} className="bg-slate-950 rounded-xl p-3"><div className="text-sm font-semibold">{item.name || item.title}</div><div className="text-[10px] text-slate-500 font-mono">{item.code || item.gradeLevel}</div></div>)}{!items.length && <p className="text-xs text-slate-500">None configured.</p>}</div></section>)}</div>
    </div>}
