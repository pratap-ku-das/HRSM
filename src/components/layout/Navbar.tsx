...
export const Navbar: React.FC<NavbarProps> = ({ onNavigateToPublic, setActiveView, onOpenCommandPalette, onToggleMobileSidebar }) => {
  ...
  const canManageWorkspace = currentUser?.permissions?.includes('company.manage') === true;
  ...
  return <header className="app-navbar h-16 border-b border-slate-800/80 bg-slate-900/80 backdrop-blur-2xl sticky top-0 z-30 px-3 sm:px-5 md:px-6 flex items-center justify-between">
    ...
    <div className="flex items-center gap-2">
      ...
      <div className="relative"><button onClick={() => setShowUserMenu(value => !value)} className="flex items-center gap-2 p-1.5 rounded-xl bg-slate-800 border border-slate-700"><img src={currentUser?.avatarUrl || '/logo.svg'} alt="" className="w-7 h-7 rounded-lg object-cover" /><ChevronDown className="w-3" /></button>{showUserMenu && <><button className="fixed inset-0 z-40" onClick={() => setShowUserMenu(false)} aria-label="Close menu" /><div className="absolute right-0 mt-2 z-50 w-56 rounded-2xl bg-slate-900 border border-slate-700 p-2 shadow-2xl"><div className="px-3 py-2 border-b border-slate-800"><strong className="block text-xs">{currentUser?.fullName}</strong><span className="text-[10px] text-slate-400">{currentUser?.email}</span><span className="mt-1 block text-[9px] uppercase tracking-wider text-brand-300">{currentUser?.role.replaceAll("_", " ")}</span></div><button onClick={() => { setActiveView(canManageWorkspace ? 'settings' : 'security-center'); setShowUserMenu(false); }} className="w-full flex gap-2 p-3 text-xs"><UserIcon className="w-4" />{canManageWorkspace ? 'Workspace settings' : 'My security'}</button><button onClick={() => { onNavigateToPublic(); setShowUserMenu(false); }} className="w-full flex gap-2 p-3 text-xs"><Globe className="w-4" />Public site</button><button onClick={signOut} className="w-full flex gap-2 p-3 text-xs text-rose-400"><LogOut className="w-4" />Sign out</button></div></>}</div>
    </div>
  </header>;
};
