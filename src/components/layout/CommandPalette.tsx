...
export const CommandPalette: React.FC<Props> = ({ isOpen, onClose, setActiveView }) => {
  ...
  const items = useMemo(() => {
    ...
    const local = navigation.filter(([route, title]) => canAccessView(currentUser, route) && (!q || title.toLowerCase().includes(q))).map(([route, title]) => ({ id: route, title, subtitle: 'Open module', route }));
    return [...remote.map(item => ({ ...item, id: `${item.type}-${item.id}` })), ...local];
  }, [currentUser, query, remote]);
