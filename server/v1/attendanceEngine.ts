...
export function evaluateAttendance(input: AttendanceEvaluationInput) {
  const { clockIn, clockOut, policy } = input; const flags: string[] = [];
  if (!clockIn) flags.push('MISSING_CLOCK_IN'); if (!clockOut) flags.push('MISSING_CLOCK_OUT');
  ...
  let workedMinutes = 0; if (clockIn && clockOut) workedMinutes = Math.max(0, Math.round((clockOut.getTime() - clockIn.getTime()) / 60_000) - input.breakMinutes);
  let inMinute = clockIn ? minuteOfDay(clockIn) : policy.startMinute; let outMinute = clockOut ? minuteOfDay(clockOut) : policy.endMinute; if (policy.crossesMidnight && outMinute < policy.startMinute) outMinute += 1440;
  ...
  if (lateMinutes) flags.push('LATE'); if (earlyExitMinutes) flags.push('EARLY_EXIT'); if (workedMinutes > 0 && workedMinutes < policy.halfDayAfterMinutes) flags.push('INSUFFICIENT_HOURS'); if (overtimeMinutes) flags.push('OVERTIME');
  const status = !clockIn ? 'ABSENT' : workedMinutes < policy.halfDayAfterMinutes ? 'HALF_DAY' : lateMinutes ? 'LATE' : 'PRESENT';
